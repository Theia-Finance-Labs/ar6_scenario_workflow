...
def ultra_fast_gap_fill(df: pd.DataFrame, lookup_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for df_col in gap_fillable_columns:
        # Create column if it doesn't exist (initialize with NaN)
        if df_col not in df.columns:
            df[df_col] = np.nan
            print(f"  ⚡ Processing {df_col}... (created column)")
        else:
        ...
        if lookup_col not in lookup_df.columns:
            print(
                f"    ⚠️  Lookup column {lookup_col} not found in lookup table, skipping"
            )
            continue
        ...
        direct_filled = direct_technology_match(
            missing_df, lookup_df, df_col, lookup_col
        )
        ...
        if len(missing_df) > 0:
            ...
            for base_tech, fallback_tech in tech_fallbacks.items():
                ...
                if len(tech_missing) > 0:
                    fallback_filled = direct_technology_match(
                        tech_missing,
                        lookup_df,
                        df_col,
                        lookup_col,
                        override_tech=fallback_tech,
                    )
                    if len(fallback_filled) > 0:
                        df.loc[fallback_filled.index, df_col] = fallback_filled
                        df.loc[fallback_filled.index, "gap_filled_columns"] = (
                            update_tracking(
                                df.loc[fallback_filled.index, "gap_filled_columns"],
                                f"{df_col}(tech_fallback)",
                            )
                        )
                        col_filled += len(fallback_filled)
                        filled_counts[df_col] += len(fallback_filled)
                        print(
                            f"      ✅ {base_tech}->{fallback_tech}: {len(fallback_filled)} values"
                        )
        ...
        if len(missing_df) > 0:
            print(
                f"    ⛽ Strategy 2b: Cross-fossil fallbacks (Oil -> Coal+Gas average)..."
            )

            oil_techs_missing = missing_df[
                missing_df["technology"].str.contains("Oil", na=False)
            ]
            if len(oil_techs_missing) > 0:
                cross_fossil_filled = cross_fossil_fallback_vectorized(
                    oil_techs_missing, lookup_df, df_col, lookup_col
                )
                if len(cross_fossil_filled) > 0:
                    df.loc[cross_fossil_filled.index, df_col] = cross_fossil_filled
                    df.loc[cross_fossil_filled.index, "gap_filled_columns"] = (
                        update_tracking(
                            df.loc[cross_fossil_filled.index, "gap_filled_columns"],
                            f"{df_col}(oil_coal_gas_avg)",
                        )
                    )
                    col_filled += len(cross_fossil_filled)
                    filled_counts[df_col] += len(cross_fossil_filled)
                    print(
                        f"      ✅ Oil->Coal+Gas average: {len(cross_fossil_filled)} values"
                    )
        ...
        if len(missing_df) > 0:
            ...
            global_filled = global_fallback_match(
                missing_df, lookup_df, df_col, lookup_col
            )
            if len(global_filled) > 0:
                ...
                df.loc[global_filled.index, "gap_filled_columns"] = update_tracking(
                    df.loc[global_filled.index, "gap_filled_columns"],
                    f"{df_col}(global)",
                )
        ...
        if len(missing_df) > 0 and df_col == "lifetime_years":
            print(
                f"    🔧 Strategy 4: Hardcoded fallbacks for systematic missing data..."
            )

            # OilCap lifetime: calculated mean from GasCap+CoalCap (65,620 data points)
            OILCAP_LIFETIME_FALLBACK = 36.6

            # Solar PV lifetime: median from 28,137 data points across all models
            SOLAR_LIFETIME_FALLBACK = 30.0

            # Wind lifetime: median from 17,659 data points across all models
            WIND_LIFETIME_FALLBACK = 30.0
            ...
            wind_variants = ["WindCap", "WindCap - Onshore", "WindCap - Offshore"]
            ...
            for oil_tech in oilcap_variants:
                ...
                if len(oil_missing) > 0:
                    ...
                    df.loc[oil_missing.index, "gap_filled_columns"] = update_tracking(
                        df.loc[oil_missing.index, "gap_filled_columns"],
                        f"{df_col}(hardcoded_fallback)",
                    )
                    col_filled += len(oil_missing)
                    filled_counts[df_col] += len(oil_missing)
                    print(
                        f"      ✅ {oil_tech} hardcoded fallback: {len(oil_missing)} values ({OILCAP_LIFETIME_FALLBACK} years)"
                    )

            for solar_tech in solar_variants:
                solar_missing = missing_df[missing_df["technology"] == solar_tech]
                if len(solar_missing) > 0:
                    df.loc[solar_missing.index, df_col] = SOLAR_LIFETIME_FALLBACK
                    df.loc[solar_missing.index, "gap_filled_columns"] = update_tracking(
                        df.loc[solar_missing.index, "gap_filled_columns"],
                        f"{df_col}(hardcoded_fallback)",
                    )
                    col_filled += len(solar_missing)
                    filled_counts[df_col] += len(solar_missing)
                    print(
                        f"      ✅ {solar_tech} hardcoded fallback: {len(solar_missing)} values ({SOLAR_LIFETIME_FALLBACK} years)"
                    )

            for wind_tech in wind_variants:
                wind_missing = missing_df[missing_df["technology"] == wind_tech]
                if len(wind_missing) > 0:
                    df.loc[wind_missing.index, df_col] = WIND_LIFETIME_FALLBACK
                    df.loc[wind_missing.index, "gap_filled_columns"] = update_tracking(
                        df.loc[wind_missing.index, "gap_filled_columns"],
                        f"{df_col}(hardcoded_fallback)",
                    )
                    col_filled += len(wind_missing)
                    filled_counts[df_col] += len(wind_missing)
                    print(
                        f"      ✅ {wind_tech} hardcoded fallback: {len(wind_missing)} values ({WIND_LIFETIME_FALLBACK} years)"
                    )
...
def direct_technology_match(
    missing_df: pd.DataFrame,
    lookup_df: pd.DataFrame,
    df_col: str,
    lookup_col: str,
    override_tech: str = None,
) -> pd.Series:
    ...
    if override_tech:
        ...
        lookup_subset = lookup_df[
            (lookup_df["technology"] == override_tech) & (lookup_df[lookup_col].notna())
        ]
        ...
        results = []
        for (year, stringency), year_group in missing_df.groupby(
            ["scenario_year", "stringency"]
        ):
            ...
            for idx in year_group.index:
                ...
                if not country_list or country_list == "nan":
                    ...
                    countries = [
                        c.strip() for c in country_list.split(",") if c.strip()
                    ]
        ...
        for tech, tech_group in missing_df.groupby("technology"):
            tech_lookup = lookup_df[
                (lookup_df["technology"] == tech) & (lookup_df[lookup_col].notna())
            ]
            ...
            # Group by year/stringency for efficiency
            for (year, stringency), year_group in tech_group.groupby(
                ["scenario_year", "stringency"]
            ):
                matches = tech_lookup[
...
def cross_fossil_fallback_vectorized(
    oil_techs_missing: pd.DataFrame,
    lookup_df: pd.DataFrame,
    df_col: str,
    lookup_col: str,
) -> pd.Series:
    ...
    for oil_tech, fallback_techs in tech_mapping.items():
        ...
        for (year, stringency), year_group in oil_subset.groupby(
            ["scenario_year", "stringency"]
        ):
            ...
            for idx in year_group.index:
                ...
                for country in target_countries:
                    country_matches = year_stringency_matches[
                        year_stringency_matches["iso2"] == country
                    ]
                    if len(country_matches) > 0:
                ...
                if not values_to_average:
                    global_matches = year_stringency_matches[
                        year_stringency_matches["iso2"] == "GLOBAL"
                    ]
...
def global_fallback_match(
    missing_df: pd.DataFrame, lookup_df: pd.DataFrame, df_col: str, lookup_col: str
) -> pd.Series:
