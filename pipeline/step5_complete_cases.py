...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("step5_complete_cases.log"), logging.StreamHandler()],
)
...
def check_completeness(df, essential_cols):
    ...
    for col in essential_cols:
        if col in df.columns:
            ...
            # Special handling for scenario_capacity_factor: only required for Power/Renewables
            if col == "scenario_capacity_factor":
                power_renewables_mask = df["sector"].isin(["Power", "Renewables"])
                # For Power/Renewables, capacity factor is required (must be non-null)
                # For other sectors (Gas&Oil), it's optional (can be NaN)
                # So: complete if (has value) OR (not Power/Renewables)
                col_complete = col_complete | (~power_renewables_mask)
            ...
            logging.warning(
                f"  {col}: Column not found in dataset - treating as optional"
            )
            # Don't mark all rows as incomplete if column is missing
            # Instead, just log the warning and continue
...
def handle_fuel_price_exceptions(df, complete_mask):
    ...
    logging.info(
        f"  Allowed {green_exception.sum():,} green technology cases with missing fuel prices"
    )
...
def analyze_ebitda_failures(df):
    ...
    tech_breakdown = (
        false_cases.groupby("technology").size().sort_values(ascending=False)
    )
    ...
    scenario_breakdown = (
        false_cases.groupby("scenario").size().sort_values(ascending=False)
    )
...
def filter_complete_cases(input_file, output_file):
    ...
    for sector in sorted(df["sector"].unique()):
        ...
        logging.info(
            f"  {sector}: {sector_complete:,}/{sector_total:,} ({retention:.1f}%)"
        )
    ...
    for tech in sorted(df["technology"].unique()):
        ...
        if retention < 90:
            logging.info(
                f"  {tech}: {tech_complete:,}/{tech_total:,} ({retention:.1f}%)"
            )
