...
try:
    ...
    print(f"   ✅ Loaded target schema data: {df.shape[0]:,} rows × {df.shape[1]} columns")
...
aggregation_groups = df[df['aggregation_group'] != 'single']['aggregation_group'].unique()
...
def aggregate_technology_group(group_df):
    ...
    print(f"     Aggregating {len(group_df)} rows for {mapped_technology} (framework: {decision_framework})")
...
cols_to_drop = ['current_sector_tech', 'mapped_sector', 'mapped_technology', 'aggregation_group']
...
def gap_fill_with_grouped_medians(df, columns_to_fill):
    ...
    for col in columns_to_fill:
        ...
        for level_idx, group_cols in enumerate(grouping_levels):
            ...
            existing_group_cols = [col for col in group_cols if col in df_filled.columns]
        ...
        print(f"     ✅ {col}: {filled_count:,} values filled, {final_missing:,} still missing")
...
final_combinations = aggregated_df.groupby(['sector', 'technology']).size().reset_index(name='count')
for _, row in final_combinations.iterrows():
    print(f"     {row['sector']} - {row['technology']}: {row['count']:,} rows")

# Check for any unmapped technologies
unmapped = aggregated_df[~aggregated_df.apply(lambda x: (x['sector'], x['technology']) in 
                                              [(row['target_sector'], row['target_technology']) 
                                               for _, row in mapping_df.iterrows()], axis=1)]

if len(unmapped) > 0:
    print(f"   ⚠️  Found {len(unmapped)} rows with unmapped technologies:")
    unmapped_techs = unmapped.groupby(['sector', 'technology']).size().reset_index(name='count')
...
target_cols = [
    ...
    'scenario_price', 'pathway_unit', 'scenario_pathway', 'scenario_capacity_factor',
...
if rows_with_any_missing > 0:
    print(f"   💡 Recommendation: Consider dropping {rows_with_any_missing:,} rows with missing values")
    print(f"   💡 This would leave {(len(aggregated_df) - rows_with_any_missing):,} complete rows")
else:
...
print(f"     Reduction: {df.shape[0] - len(aggregated_df):,} rows ({(df.shape[0] - len(aggregated_df))/df.shape[0]*100:.1f}%)")
...
print(f"     • Target scenarios (C1, C2): {(aggregated_df['scenario_type'] == 'target').sum():,} rows")
print(f"     • Baseline scenarios (others): {(aggregated_df['scenario_type'] == 'baseline').sum():,} rows")
...
target_cols = [
    ...
    'sector', 'technology', 'technology_type', 'price_unit', 'price_indicator',
    'scenario_price', 'pathway_unit', 'scenario_pathway', 'scenario_capacity_factor',
    'scenario_year', 'country_iso2_list', 'stringency'
]
...
efficiency_condition = (
    (final_output['sector'].isin(['Power', 'Renewables'])) & 
    (final_output['efficiency_decimal'].isna())
)

# Filter to only include columns that exist
existing_critical_columns = [col for col in critical_columns if col in final_output.columns]
print(f"   📋 Critical columns for complete case filtering: {existing_critical_columns}")
...
if 'efficiency_decimal' in final_output.columns:
    ...
    print(f"     • efficiency_decimal (Power/Renewables only): {efficiency_missing:,} missing values")
...
for sector in sorted(final_output['sector'].unique()):
    ...
    print(f"     • {sector}: {sector_completeness:.1f}% ({sector_complete_count:,}/{sector_total:,})")
...
print(f"     Total reduction: {df.shape[0] - len(final_complete):,} rows ({(df.shape[0] - len(final_complete))/df.shape[0]*100:.1f}%)")
