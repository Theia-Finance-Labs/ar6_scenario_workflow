...
try:
    ...
    print(
        f"   ✅ Loaded ISO3 data: {iso3_df.shape[0]:,} rows × {iso3_df.shape[1]} columns"
    )
...
try:
    ...
    print(
        f"   ✅ Loaded R10 data: {r10_df.shape[0]:,} rows × {r10_df.shape[1]} columns"
    )
except FileNotFoundError:
...
if datasets:
    ...
    raise FileNotFoundError(
        "No input files found! Please run script 1 first to generate the intermediate files."
    )
...
print(
    "   Target metrics: Capacity, Capacity Additions, Energy (Primary/Secondary), Lifetime"
)
...
energy_data = df_filtered[
    df_filtered["col1"].isin(["Primary Energy", "Secondary Energy"])
]
...
join_cols = [
    col for col in join_cols if col in cost_data.columns and col in pivoted.columns
]
...
def add_cost_metric(pivoted_df, cost_df, metric_name, join_cols):
    ...
    if "unit" in metric_data.columns:
        most_common_unit = (
            metric_data["unit"].mode().iloc[0]
            if not metric_data["unit"].mode().empty
            else "unknown"
        )
    ...
    if metric_name == "OM Cost":
        ...
        if "%" in most_common_unit or "percent" in most_common_unit.lower():
            ...
            unit_clean = (
                most_common_unit.replace("US$2010/", "")
                .replace("/", "_per_")
                .replace(" ", "_")
                .lower()
            )
            col_name = (
                f"{col_name_base}_{unit_clean}"
                if unit_clean != "unknown"
                else col_name_base
            )
    ...
    print(
        f"     ✅ Added {metric_name} as '{col_name}': {result[col_name].notna().sum():,} values"
    )
    return result
...
if "om_cost_unit" in cost_data.columns:
    ...
    print(
        f"     ✅ Added om_cost_unit: {pivoted['om_cost_unit'].notna().sum():,} values"
    )

if "capital_cost_unit" in cost_data.columns:
    capital_unit_data = cost_data[join_cols + ["capital_cost_unit"]].copy()
    capital_unit_data = capital_unit_data.dropna(
        subset=["capital_cost_unit"]
    ).drop_duplicates()
    pivoted = pivoted.merge(capital_unit_data, on=join_cols, how="left")
    print(
        f"     ✅ Added capital_cost_unit: {pivoted['capital_cost_unit'].notna().sum():,} values"
    )

if "efficiency_unit" in cost_data.columns:
    efficiency_unit_data = cost_data[join_cols + ["efficiency_unit"]].copy()
    efficiency_unit_data = efficiency_unit_data.dropna(
        subset=["efficiency_unit"]
    ).drop_duplicates()
    pivoted = pivoted.merge(efficiency_unit_data, on=join_cols, how="left")
    print(
        f"     ✅ Added efficiency_unit: {pivoted['efficiency_unit'].notna().sum():,} values"
    )
...
if all(col in df.columns for col in carbon_price_cols):
    ...
    if not carbon_data.empty:
        ...
        print(
            f"     ✅ Added carbon price: {pivoted['carbon_price_usd_per_tco2'].notna().sum():,} values"
        )
    else:
...
if all(col in df.columns for col in primary_price_cols):
    ...
    primary_price_data = primary_price_data.dropna(
        subset=["primary_energy_price"]
    ).drop_duplicates()
    ...
    if not primary_price_data.empty:
        ...
        primary_price_data = primary_price_data.rename(
            columns={"primary_energy_price": col_name}
        )
        primary_price_data = primary_price_data.drop(
            "primary_energy_price_unit", axis=1
        )
        ...
        merge_cols = [
            col
            for col in price_join_cols
            if col in pivoted.columns and col in primary_price_data.columns
        ]
        ...
        primary_price_unit_data = primary_price_unit_data.dropna(
            subset=["primary_energy_price_unit"]
        ).drop_duplicates()
        ...
        print(
            f"     ✅ Added primary energy price: {pivoted[col_name].notna().sum():,} values"
        )
        print(
            f"     ✅ Added primary_energy_price_unit: {pivoted['primary_energy_price_unit'].notna().sum():,} values"
        )
    else:
...
if all(col in df.columns for col in secondary_price_cols):
    ...
    secondary_price_data = secondary_price_data.dropna(
        subset=["secondary_energy_price"]
    ).drop_duplicates()
    ...
    if not secondary_price_data.empty:
        ...
        secondary_price_data = secondary_price_data.rename(
            columns={"secondary_energy_price": col_name}
        )
        secondary_price_data = secondary_price_data.drop(
            "secondary_energy_price_unit", axis=1
        )
        ...
        merge_cols = [
            col
            for col in price_join_cols
            if col in pivoted.columns and col in secondary_price_data.columns
        ]
        # Also add the unit column for row-by-row determination
        secondary_price_unit_data = df[
            merge_cols + ["secondary_energy_price_unit"]
        ].copy()
        secondary_price_unit_data = secondary_price_unit_data.dropna(
            subset=["secondary_energy_price_unit"]
        ).drop_duplicates()
        ...
        print(
            f"     ✅ Added secondary energy price: {pivoted[col_name].notna().sum():,} values"
        )
        print(
            f"     ✅ Added secondary_energy_price_unit: {pivoted['secondary_energy_price_unit'].notna().sum():,} values"
        )
    else:
...
if all(col in df.columns for col in secondary_elec_price_cols):
    ...
    secondary_elec_data = secondary_elec_data.dropna(
        subset=["secondary_energy_electricity_price"]
    ).drop_duplicates()

    if not secondary_elec_data.empty:
        # All secondary energy electricity prices are in US$2010/GJ - no unit variation
        print(
            f"     📏 Secondary energy electricity price unit: US$2010/GJ (consistent)"
        )

        col_name = "secondary_energy_electricity_price_usd_per_gj"
        secondary_elec_data = secondary_elec_data.rename(
            columns={"secondary_energy_electricity_price": col_name}
        )
        secondary_elec_data = secondary_elec_data.drop(
            "secondary_energy_electricity_price_unit", axis=1
        )

        pivoted = pivoted.merge(secondary_elec_data, on=join_cols, how="left")
        print(
            f"     ✅ Added secondary energy electricity price: {pivoted[col_name].notna().sum():,} values"
        )
    else:
...
print(
    "   No filtering applied - retaining all rows regardless of price data completeness"
)
...
if len(filtered_df) > 0:
    ...
    def safe_convert_capacity(df, unit_map):
        ...
        if original_unit == "GW":
            ...
            print(
                f"   ⚠️  Capacity: Unknown unit '{original_unit}' - values preserved as-is"
            )
    ...
    def safe_convert_capacity_additions(df, unit_map):
        ...
        original_unit = unit_map.get("Capacity Additions", {}).get(
            "most_common", "GW/yr"
        )
        ...
        if original_unit in ["GW/yr", "GW yr-1"]:
            ...
            df["capacity_additions_mw_per_yr"] = df["capacity_additions_value"]
            print(
                f"   ⚠️  Capacity Additions: Unknown unit '{original_unit}' - values preserved as-is"
            )
    ...
    def safe_convert_energy(df, unit_map, energy_type):
        ...
        if unit_col in df.columns:
            ...
            if ej_count > 0:
                ...
                print(
                    f"   ✅ {energy_type}: {ej_count:,} EJ/yr values → MWh/yr (×{ej_to_mwh:.0e})"
                )
            ...
            if pj_count > 0:
                ...
                print(
                    f"   ✅ {energy_type}: {pj_count:,} PJ/yr values → MWh/yr (×{pj_to_mwh:.0e})"
                )
            ...
            if tj_count > 0:
                ...
                print(
                    f"   ✅ {energy_type}: {tj_count:,} TJ/yr values → MWh/yr (×{tj_to_mwh:.0e})"
                )
            ...
            if "PJ" in original_unit:
                ...
                print(
                    f"   ✅ {energy_type}: {original_unit} → MWh/yr (×{pj_to_mwh:.0e})"
                )
            else:
                df[target_col] = df[value_col] * ej_to_mwh
                print(
                    f"   ✅ {energy_type}: {original_unit} → MWh/yr (×{ej_to_mwh:.0e})"
                )
    ...
    def safe_convert_lifetime(df, unit_map):
        ...
        if original_unit in ["years", "year", "yr", "y"]:
            ...
            df["lifetime_years"] = df["lifetime_value"]
            print(
                f"   ⚠️  Lifetime: Unknown unit '{original_unit}' - values preserved as-is"
            )
    ...
    if "om_cost_usd_per_mw_per_yr" in filtered_df.columns:
        ...
        if extreme_negative_mask.any():
            print(
                f"   ⚠️  Filtering out {extreme_negative_mask.sum():,} extreme negative OM costs"
            )
        ...
        reasonable_mask = (filtered_df["om_cost_usd_per_mw_per_yr"] > 0) & (
            filtered_df["om_cost_usd_per_mw_per_yr"] < 1000000000
        )
        if reasonable_mask.any():
            filtered_df.loc[reasonable_mask, "om_cost_usd_per_mw_per_yr"] = (
                filtered_df.loc[reasonable_mask, "om_cost_usd_per_mw_per_yr"] * 1000
            )
            print(
                f"   ✅ OM Cost: Applied kW→MW conversion (×1000) to {reasonable_mask.sum():,} reasonable values"
            )
        else:
    ...
    if "capital_cost_usd_per_mw" in filtered_df.columns:
        ...
        if extreme_negative_mask.any():
            print(
                f"   ⚠️  Filtering out {extreme_negative_mask.sum():,} extreme negative Capital costs"
            )
        ...
        # Apply conversion only to reasonable positive values (up to 1 billion)
        reasonable_mask = (filtered_df["capital_cost_usd_per_mw"] > 0) & (
            filtered_df["capital_cost_usd_per_mw"] < 1000000000
        )
        if reasonable_mask.any():
            filtered_df.loc[reasonable_mask, "capital_cost_usd_per_mw"] = (
                filtered_df.loc[reasonable_mask, "capital_cost_usd_per_mw"] * 1000
            )
            print(
                f"   ✅ Capital Cost: Applied kW→MW conversion (×1000) to {reasonable_mask.sum():,} reasonable values"
            )
        else:
    ...
    if "efficiency_percent" in filtered_df.columns:
        ...
        if zero_efficiency_mask.any():
            print(
                f"   ⚠️  Setting {zero_efficiency_mask.sum():,} zero efficiency values to NaN"
            )
        ...
        if efficiency_mask.any():
            filtered_df.loc[efficiency_mask, "efficiency_decimal"] = (
                filtered_df.loc[efficiency_mask, "efficiency_percent"] / 100
            )
            print(
                f"   ✅ Efficiency: {efficiency_mask.sum():,} values converted from % to decimal"
            )
        else:
    ...
    energy_price_fields = [
        "primary_energy_price_usd_per_gj",
        "secondary_energy_price_usd_per_gj",
        "secondary_energy_electricity_price_usd_per_gj",
    ]
    ...
    energy_cols_new = [
        col for col in filtered_df.columns if "energy" in col and "mwh_per_yr" in col
    ]
    if energy_cols_new:
        ...
        print(
            f"     ⚡ Technologies with energy data: {techs_with_energy}/{total_techs}"
        )
        ...
        for tech in sorted(filtered_df["Technology"].unique()):
            ...
            primary_count = (
                tech_data["primary_energy_mwh_per_yr"].notna().sum()
                if "primary_energy_mwh_per_yr" in tech_data.columns
                else 0
            )
            secondary_count = (
                tech_data["secondary_energy_mwh_per_yr"].notna().sum()
                if "secondary_energy_mwh_per_yr" in tech_data.columns
                else 0
            )
            energy_status = []
            if primary_count > 0:
            ...
            energy_str = (
                " + ".join(energy_status) if energy_status else "No energy data"
            )
    ...
    grouping_cols_final = [
        ...
        for col in all_cols
        if col
        in ["model", "scenario", "scenario_geography", "year", "Sector", "Technology"]
    ]
    value_cols_final = [col for col in all_cols if col not in grouping_cols_final]

    print(
        f"     📊 Grouping columns ({len(grouping_cols_final)}): {grouping_cols_final}"
    )
