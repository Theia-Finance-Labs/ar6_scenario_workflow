...
try:
    ...
    print(
        f"   Loaded step 1 data: {step1_df.shape[0]:,} rows × {step1_df.shape[1]} columns"
    )
...
if "capacity_mw" in df.columns:
    ...
    print(
        f"     • Non-null values: {capacity_non_null:,} ({capacity_completeness:.1f}%)"
    )
    print(
        f"     • Missing/null values: {capacity_null:,} ({100-capacity_completeness:.1f}%)"
    )

    # Check completeness by sector
    print(f"   📊 Capacity data completeness by sector:")
    sector_completeness = df.groupby("Sector")["capacity_mw"].agg(
        ["count", lambda x: x.notna().sum(), lambda x: x.notna().sum() / len(x) * 100]
    )
    sector_completeness.columns = ["total_rows", "non_null", "completeness_pct"]
    for sector, row in sector_completeness.iterrows():
        print(
            f"     • {sector}: {row['non_null']:,}/{row['total_rows']:,} ({row['completeness_pct']:.1f}%)"
        )

    # Check if there are any non-zero values
    non_zero_capacity = df[df["capacity_mw"].notna() & (df["capacity_mw"] > 0)][
        "capacity_mw"
    ].count()
...
print(
    f"   💡 Coal/Gas&Oil → primary energy | Power/Renewables → technology-specific rules"
)
...
try:
    ...
    print(
        f"   Loaded metadata: {metadata_df.shape[0]:,} rows with columns: {list(metadata_df.columns)}"
    )

    # Create a lookup dictionary from Model+Scenario to Category
    if (
        "Model" in metadata_df.columns
        and "Scenario" in metadata_df.columns
        and "Category" in metadata_df.columns
    ):
        # Clean the metadata
        metadata_clean = metadata_df[["Model", "Scenario", "Category"]].dropna()
        metadata_clean["lookup_key"] = (
            metadata_clean["Model"].astype(str)
            + "|||"
            + metadata_clean["Scenario"].astype(str)
        )
        ...
        print(
            f"   Created lookup with {len(category_lookup):,} Model-Scenario combinations"
        )
        ...
        total = len(target_df)
        print(
            f"   ✅ Matched {matched:,}/{total:,} rows ({matched/total*100:.1f}%) with metadata categories"
        )
        ...
        print(
            "   ❌ Required columns (Model, Scenario, Category) not found in metadata"
        )
    ...
    print(
        "   ⚠️  Setting scenario_type to empty - place the Excel file in the working directory"
    )
    target_df["scenario_type"] = ""
...
target_df["pathway_unit"] = target_df.apply(
    lambda row: determine_pathway_unit(row, df), axis=1
)
...
target_df["scenario_pathway"] = target_df.apply(
    lambda row: calculate_scenario_pathway(row, df), axis=1
)
...
target_df["scenario_capacity_factor"] = target_df.apply(
    lambda row: calculate_capacity_factor(row, df), axis=1
)

# Handle division by zero and invalid values
target_df["scenario_capacity_factor"] = target_df["scenario_capacity_factor"].replace(
    [np.inf, -np.inf], np.nan
)

# Report pathway calculation results by sector
print("   Pathway calculation results by sector:")
pathway_completeness = target_df.groupby("sector")["scenario_pathway"].agg(
    ["count", lambda x: x.notna().sum(), lambda x: x.notna().sum() / len(x) * 100]
)
...
for sector, row in pathway_completeness.iterrows():
    if sector in ["Coal", "Gas&Oil"]:
        print(
            f"     • {sector} (Rule 1 - primary energy): {row['non_null']:,}/{row['total_rows']:,} ({row['completeness_pct']:.1f}%)"
        )
    else:
        ...
        print(
            f"     • {sector} (data availability-driven): {row['non_null']:,}/{row['total_rows']:,} ({row['completeness_pct']:.1f}%)"
        )
...
if len(valid_cf) > 0:
    ...
    print(
        f"     📊 Values below 0: {below_zero:,} ({below_zero/len(valid_cf)*100:.1f}%)"
    )
    print(f"     📊 Values above 1: {above_one:,} ({above_one/len(valid_cf)*100:.1f}%)")
    print(
        f"     📊 Values in normal range [0-1]: {normal_range:,} ({normal_range/len(valid_cf)*100:.1f}%)"
    )
...
# Apply clipping
target_df["scenario_capacity_factor"] = target_df["scenario_capacity_factor"].clip(
    0, 1
)  # Cap at 100%
...
if len(cf_after_clip) > 0:
    print(
        f"     ✅ After clipping - Range: {cf_after_clip.min():.3f} to {cf_after_clip.max():.3f}"
    )
    print(
        f"     ✅ After clipping - Mean: {cf_after_clip.mean():.3f}, Median: {cf_after_clip.median():.3f}"
    )
...
def create_iso3_to_iso2_mapping():
    ...
    print(
        f"   📊 Total mappings created: {len(full_mapping)} ({len(country_mapping)} countries + {len(regional_mapping)} regions + {len(r10_mapping)} R10 regions)"
    )
...
target_df["country_iso2_list"] = target_df["scenario_geography"].apply(
    map_geography_to_iso2
)
...
print(
    f"   ✅ Successfully mapped {total_mapped_geographies}/{total_geographies} geographies ({mapping_success_rate:.1f}%) to ISO2 codes"
)
print(
    f"   📊 Geography breakdown: {len(iso3_countries)} ISO3 countries, {len(other_regions)} regions, {len(r10_regions)} R10 regions"
)

if unmapped_geographies:
    total_unmapped_rows = sum(
        (target_df["scenario_geography"] == geo).sum()
        for geo, _ in unmapped_geographies
    )
    print(
        f"   ⚠️  {len(unmapped_geographies)} unmapped geographies affecting {total_unmapped_rows:,} rows"
    )

print(
    f"   Created target schema: {target_df.shape[0]:,} rows × {target_df.shape[1]} columns"
)
...
print(
    f"     📊 scenario_pathway: {target_df['scenario_pathway'].notna().sum():,} values"
)
print(
    f"     📊 scenario_capacity_factor: {target_df['scenario_capacity_factor'].notna().sum():,} values"
)
...
print(
    f"     📊 Years: {final_df['scenario_year'].min()}-{final_df['scenario_year'].max()}"
)
